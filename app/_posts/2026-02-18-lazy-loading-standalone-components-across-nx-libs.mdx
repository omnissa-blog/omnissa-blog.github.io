...
Product and engineering teams often need the same view, modal, or component to appear in different places — for example, when visualizing a relationship between entities that live in different libraries of the [Nx monorepo](https://nx.dev/docs/concepts/decisions/why-monorepos). Reusing the same UI in these contexts keeps behavior and UX consistent, but it can conflict with keeping bundles small and avoiding circular dependencies across Nx libraries.

This article describes a pattern for Lazy Loading Angular Standalone Components from one Nx library into another. That approach enables code splitting and on-demand loading of feature components across library boundaries, so the same component can be rendered in multiple contexts without duplicating logic or pulling heavy dependencies in every consumer.
...
- **Bundle bloating:** All of Feature B's code gets bundled with Feature A, even if only used occasionally to serve a specific use case. When Vite/esbuild builds the application, it must include Feature B's entire codebase, including all its components, services, NgRx Store modules, and dependencies in Feature A's bundle, even if that component is rarely used or only needed in specific scenarios.
...
export class FeatureADashboardComponent {
  ...
  public async lazyLoadComponent(entity: Entity): Promise<void> {
    ...
    // Step 5: Pass Input props to the dynamically created component
    componentRef?.setInput('entityId', entity?.id);
    componentRef?.setInput('displayMode', 'modal');

    // Step 6: Listen to Output events from the dynamically created component
...
- Going against Modern Angular's direction (not compatible with standalone components)
...
> **Note:** `setComponentImportByType` can be called directly on `app.component.ts` initialization or in `app.module.ts`.
...
export class LazyLoadService {
  ...
  public getComponentType(componentType: LazyComponentType): Signal<Type<unknown> | null> {
    ...
    const componentTypeSig = signal<Type<unknown> | null>(null);

    // Start loading the component
    this.importByType?.[componentType]?.().then((type) => {
      componentTypeSig.set(type);
    });

    // Cache the signal
    this.componentTypeCache.set(componentType, componentTypeSig);
    return componentTypeSig;
  }
}
```
...
@Component({
  ...
  template: `
    @if (loadedComponentTypeSig()) {
      <ng-container *ngComponentOutlet="loadedComponentTypeSig(); inputs: propsSig()" />
    }
  `,
  imports: [NgComponentOutlet],
})
export class LazyContainerComponent {
  public componentTypeSig = input<LazyComponentType>(undefined, { alias: 'componentType' });
  public propsSig = input<ComponentInputs>(undefined, { alias: 'props' });
  private readonly lazyLoadService = inject(LazyLoadService);

  public readonly loadedComponentTypeSig = computed(() => {
    const type = this.componentTypeSig();
...
- **Improved Time to Interactive / Total Blocking Time (TTI/TBT):** Less JavaScript to parse and compile means the main thread is freed up sooner for user interactions.

- **Better Core Web Vitals scores:** Lazy loading directly improves Largest Contentful Paint (LCP) and First Input Delay (FID/INP).
...
- Use `fakeAsync` and `tick()`: Since component loading is asynchronous, use the fakeAsync test zone and `tick()` to advance time until promises resolve.
- **Test signal initial state:** Verify that signals initially return `null` before the dynamic import completes.
- **Test signal resolution:** After `tick()`, verify the signal resolves to the expected component type.
...
- **Test computed signal behavior:** Verify that the `loadedComponentTypeSig` computed signal correctly derives its value from the service signal.
